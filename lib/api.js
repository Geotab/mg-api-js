...
*/
var GeotabApi = function (getCredentials, newOptions, customCredentialStore) {
  'use strict'
  var JSONP_REQUESTS_PROPERTY_STR = 'geotabJSONP',
    ...
    callBase = function (method, params, callbackSuccess, callbackError) {
      if (options.jsonp) {
        callJSONP(method, params, callbackSuccess, callbackError)
      } else {
        callXHR(method, params, callbackSuccess, callbackError)
      }
    },
    /**
    ...
    populateAuthenticationCallback = function (callbackSuccess) {
      ...
      if(typeof getCredentials === 'function'){
        ...
        getCredentials(function (newServer, database, username, password, error) {
          // Authenticating against the server with the collected information
          authenticateWithServer(newServer, database, username, password, function () {
      ...
      } else { // Was not given a callback, just credentials
        let cred = getCredentials; // Purely convenience
        authenticateWithServer(cred.server, cred.database, cred.userName, cred.password, function() {
          if(callbackSuccess){
            callbackSuccess();
          }
    ...
    call = function (method, params, callbackSuccess, callbackError) {
      ...
      callBase(method, params, callbackSuccess, function (errorString, errorObject) {

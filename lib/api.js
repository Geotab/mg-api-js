...
// JSONP appends to window. Window has to be defined to compile in node
...
*  })
*
*  You can also pass the Api object a credentials object
*  NOTE: while not necessary, the error callback allows you to see if your authentication has failed or not
*        Authentication is called upon first request to the api
*  @example
*    let error;
...
const GeotabApi = function (getCredentials, newOptions, customCredentialStore) {
  ...
  let options = {
    ...
    /**
     * Creates the method call URL using provided authentication and call information
     * @private
    ...
    /**
     *  Cleans a JSONP request from the browser -> Removes script tag added to complete
     *    the request via it's UID
     *  @private
    ...
    /**
     *  Constructs a JSONP request for a Geotab API call
     *    - Appends the script to the DOM
     *    - Adds reference to that script in the window[JSONP_REQUESTS...] object
     *    - Runs timer to check back on results of the call
     *    - Runs cleanup call once call is complete
     *  @private
    ...
    const callJSONP = function (method, params, callbackSuccess, callbackError) {
      let JSONPromise = new Promise( (resolve, reject) => {
        ...
        // Adding the JSONP request script tag to the body of the DOM
    ...
    const callBase = function (method, params, callbackSuccess, callbackError) {
      ...
      if (options.jsonp) {
        ...
        response = callAxios(method, params, callbackSuccess, callbackError);
      }
  ...
  // Mainly for node - localstorage module doesn't seem to like updating by itself
  if(!options.rememberMe){
    localStorage.clear();
  }

  // Creates a global (window) property for managing open JSONP requests
  ...
  if(typeof getCredentials === 'object' && !options.jsonp){
    ...
    let initialAuthentication = new Promise( (resolve, reject) => {
      ...
      // Calling get credentials here allows any old legacy applications using login
      // boxes to load in the event listeners associated with that
      getCredentials(function(newServer, database, username, password, error){
        cred = {
          server: newServer,
          database: database,
